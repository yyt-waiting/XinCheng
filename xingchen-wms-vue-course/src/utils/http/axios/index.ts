...
import { getToken, getTenantId,getWarehouseId } from '/@/utils/auth';
...
const transform: AxiosTransform = {
  ...
  requestInterceptors: (config: Recordable, options) => {
    ...
    let warehourseId: string | number = getWarehouseId();
    ...
    if (token && (config as Recordable)?.requestOptions?.withToken !== false) {
      ...
      config.headers[ConfigEnum.WAREHOUSE_ID] = warehourseId;
