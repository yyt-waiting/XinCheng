<template>
 <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :searchInfo="searchInfo">
      ...
      <WmsOutOrdersModal @register="registerModal" @success="handleSuccess"/>
   </div>
</template>
...
<script lang="ts" setup>
  ...
  import {wmsOutOrdersColumns} from './WmsWaveMaster.data';
  import {wmsOutOrdersList, wmsOutOrdersDelete, wmsOutOrdersDeleteBatch, wmsOutOrdersExportXlsUrl, wmsOutOrdersImportUrl } from './WmsWaveMaster.api';
  ...
  import {downloadFile} from '/@/utils/common/renderUtils';
    ...
    const {prefixCls, tableContext, onImportXls, onExportXls} = useListPage({
        ...
        importConfig: {
            url: ()=>{
                return wmsOutOrdersImportUrl + '/' + unref(mainId)
            }
        }
    });
    ...
    watch(mainId, () => {
         searchInfo['waveId'] = unref(mainId);
         reload();
      }
    );
    ...
    async function handleDelete(record) {
        await wmsOutOrdersDelete({id: record.id}, handleSuccess);
    }
    ...
    async function batchHandleDelete() {
        await wmsOutOrdersDeleteBatch({ids: selectedRowKeys.value}, handleSuccess)
    }
