  ...
  <div class="p-2 cgformErpList">
    <div class="content">
      ...
      <BasicTable @register="registerTable" :rowSelection="rowSelection">
        <!--插槽:table标题-->
        <template #tableTitle>
          <a-button type="primary"   @click="completePickTaskHandler" preIcon="ant-design:plus-outlined"> 完成拣货</a-button>
...
<script lang="ts" name="wave-wmsWaveMaster" setup>
  ...
  import {
    ...
    getImportUrl,
    getExportUrl,
    createPickTask, createShipment, listByCreate, listByPicking, completePickTask
   ...
   * 创建拣货任务
   */
  async function completePickTaskHandler() {
    //如果没有选择数据则提示
    if (selectedRowKeys.value.length === 0) {
      createMessage.warn('请选择波次')
       return;
    }
    await completePickTask({ids: selectedRowKeys.value}, handleSuccess)
  }
  /**
   * 创建包裹
   */
