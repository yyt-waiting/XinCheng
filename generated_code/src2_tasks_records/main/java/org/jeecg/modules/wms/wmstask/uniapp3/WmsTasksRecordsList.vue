...
layout: 'default',
style: {
navigationBarTitleText: '任务执行记录表',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="任务执行记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './WmsTasksRecordsData';
defineOptions({
  name: 'WmsTasksRecordsList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/wmstask/wmsTasksRecords/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/wmstask/wmsTasksRecords/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
