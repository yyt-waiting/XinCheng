...
<template>
  <PageLayout navTitle="任务表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './WmsTasksData';
defineOptions({
  name: 'WmsTasksList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/wmstask/wmsTasks/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/wmstask/wmsTasks/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
