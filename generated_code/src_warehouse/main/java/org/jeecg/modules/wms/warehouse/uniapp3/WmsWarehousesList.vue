...
<template>
  <PageLayout navTitle="仓库表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './WmsWarehousesData';
defineOptions({
  name: 'WmsWarehousesList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/warehouse/wmsWarehouses/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/warehouse/wmsWarehouses/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
