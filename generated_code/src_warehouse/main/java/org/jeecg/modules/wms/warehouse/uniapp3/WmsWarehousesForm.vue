      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['warehouseCode']"
                  :label="get4Label('仓库代码')"
                  name='warehouseCode'
                  prop='warehouseCode'
                  placeholder="请选择仓库代码"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['warehouseName']"
                  :label="get4Label('仓库名称')"
                  name='warehouseName'
                  prop='warehouseName'
                  placeholder="请选择仓库名称"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('仓库属性')"
                 labelWidth="100px"
                 type="list"
                 name='warehouseAttr'
                 dict="warehouse_attr"
                v-model="myFormData['warehouseAttr']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('状态: 创建,启动,禁用')"
                 labelWidth="100px"
                 type="list"
                 name='status'
...
const dataId = ref('')
const backRouteName = ref('WmsWarehousesList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/warehouse/wmsWarehouses/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/warehouse/wmsWarehouses/edit':'/warehouse/wmsWarehouses/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
