...
<template>
  <PageLayout navTitle="储位表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './WmsStorageLocationsData';
defineOptions({
  name: 'WmsStorageLocationsList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/warehouse/wmsStorageLocations/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/warehouse/wmsStorageLocations/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
