...
layout: 'default',
style: {
navigationBarTitleText: '库存变更表',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="库存变更表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './WmsInventoryTransData';
defineOptions({
  name: 'WmsInventoryTransList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/inventory/wmsInventoryTrans/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/inventory/wmsInventoryTrans/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
