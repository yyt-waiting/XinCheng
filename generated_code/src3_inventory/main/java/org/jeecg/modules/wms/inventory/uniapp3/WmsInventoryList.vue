...
import {columns} from './WmsInventoryData';
defineOptions({
  name: 'WmsInventoryList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/inventory/wmsInventory/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/inventory/wmsInventory/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
